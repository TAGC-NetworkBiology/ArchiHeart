...
# be use in later GWAS analysis
# ####################################################################################

## @knitr prepare_gwas_files

DATA_TO_ANALYZE = match.arg( DATA_TO_ANALYZE, c( "MEAN", "CV"))

cat("<BR><H3>Preparing data for GWAS analysis for", DATA_TO_ANALYZE, "</H3>")
...
gwas_output_dir = file.path( OUTPUT_DIR, GWAS_OUTPUT_SUBFOLDER)
dir.create( gwas_output_dir, showWarnings = FALSE, recursive = TRUE, mode = "0777")
cat("<BR>Output folder =", gwas_output_dir)
...
for( current_age in AGE_SET){
  ...
  information_gwas_preparation_df = data.frame()
  for( current_phenotype in selected_phenotype_set){
    
    # Get the means or CVs of the current phenotype for current age
    ...
    write.table( phenotype_df, file = get_phenotype_stat_file( gwas_output_dir, current_phenotype, current_age, DATA_TO_ANALYZE), quote = FALSE, col.names = FALSE, row.names = FALSE, sep=" ")

    # Export the families used
    write.table( phenotype_df[,1:2], file = get_pheno_age_families_file( gwas_output_dir, current_phenotype, current_age, DATA_TO_ANALYZE), quote = FALSE, col.names = FALSE, row.names = FALSE, sep=" ")
    
    # Build the information dataframe
    information_gwas_preparation_df = rbind( information_gwas_preparation_df, 
  ...
  count = count + 1
  dt_list[[ count]] = datatable( information_gwas_preparation_df, 
                                 caption = paste("Information on GWAS data preparation for", DATA_TO_ANALYZE, "at age", current_age),
                                 width = 3000)
}
